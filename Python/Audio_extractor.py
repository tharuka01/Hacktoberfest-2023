...
class Mp3extractor:
    def __init__(self, root):
        self.main = root
        self.main_frame = Frame(root, height=20, width=80)  # First Frame
        self.main_frame.pack(side=TOP, fill=BOTH, padx=10, pady=10)
        self.second_frame = Frame(self.main, width=100)
        self.second_frame.pack(side=BOTTOM, fill=BOTH,padx=10)  # A second frame which holds cancel and convert Buttons.
    ...
    def content(self):
        ...
        # Button to locate audio file to be saved.
        Button(self.main_frame, text="Browse..", bg="sky Blue", command=self.filesave).pack(side=TOP,anchor='e')

        empty_space2 = Label(self.main, text="_"*60,
                             height=3, fg="red")
    ...
    def file_convert(self, video_file, audio_file):  # Here video_file and audio_file are the file paths.
        ...
        audio = video.audio  # Main conversion from video to audio.
        audio.write_audiofile(audio_file)
        popup = Toplevel()  # A popup window saying completed.
        popup.title("completed!")
        Label(popup, text="Successfully converted and saved to your location \n\n\n", height=10).pack(side=TOP,
                                                                                                     anchor='nw')
        popup.mainloop()
